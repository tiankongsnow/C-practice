...
int main(){
    ...
    char saveWord[MAXLINE][MAXWORD]; //取出单词存在这里, 相当于char *saveWord[MAXLINE]
    char lowerWord[MAXLINE][MAXWORD]; //将单词都转为小写
    ...
    while(fgets(line, MAXLINE, stdin)){
        ...
        while((ptr = getword(ptr, word)) != NULL){
            strcpy(saveWord[wordCount], word);
            wordCount++; //单词总数量
            printf("%s\n", word);
        ...
        for(k = 0; k < wordCount; k++){
            for(j = 0; j < MAXWORD; j++){
                lowerWord[k][j] = tolower(saveWord[k][j]); //把每个字母转成小写
            }
        }
        printf("transform to lowerword: ");
        for(k = 0; k < wordCount; k++){
            printf("%s ", lowerWord[k]); //打印转成小写后的单词
        }
        printf("\n");
